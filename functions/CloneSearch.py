...
sys.path.insert(0, '.')
from functions.get_params import get_sigma_and_beta
from functions.normalisation_functions import noise_by_size_norm, log10_transform
from functions.gaussian_outliers import find_gaussian_outliers_fixed_sigma
...
def CloneSearch(X_counts, 
    ...
    which_beta = use a constant beta parameter or a constant B parameter in g(f) for all samples in timeseries - alternatives: constantB, constantBeta. We recommend the constantBeta setting
    ...
    if which_QC == 'strictQC':
        ...
        mask = (X_counts > 0).sum(axis=1) > 0 # all clones - assume user wants to run on everything
    else:
        raise ValueError('The parameter which_QC has an unrecognised value. Please choose one of [strictQC, looseQC, noQC]') 
    ...
    if which_transform == 'g(f)':
        ...
        sigma, fit_b = get_sigma_and_beta(freq_info, sample_order, tp_dict)
    ...
    pca_fit, R_thresh, outlier_vector = pca_outlier_identification(X_transformed_norm, qc_clones, pval_or_fdr, statistical_threshold)
    outlier_list = qc_clones[outlier_vector]

    return outlier_list, pca_fit, R_thresh, X_transformed

def CloneSearch_clustering(input_df, distance_metric = 'correlation', linkage_method = 'average', distance_thresh = 0.61):

    '''
    input_df is what we are clustering on. 
        input_df should be of size MxN where M is the number of TCRs to cluster and N is the number of timepoints or PCA dimensions.
        For distance_metric = 'correlation', the transformed and normalised frequencies are recommended. 
    ...
    We find distance_metric = 'correlation' and linkage_method = 'average' to be a good combination for transformed frequencies and 
        distance_metric = 'cosine' and linkage_method = 'complete' to be a good combination for PCA-transformed data
    '''
